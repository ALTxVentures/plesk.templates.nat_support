    ...
    <IfModule mod_rewrite.c>
        ...
        <?php foreach ($VAR->domain->webAliases AS $alias): ?>
            <?php if ($alias->isSeoRedirect) : ?>
                RewriteCond %{HTTP_HOST} ^<?php echo $alias->asciiName ?>$ [NC,OR]
                RewriteCond %{HTTP_HOST} ^www.<?php echo $alias->asciiName ?>$ [NC]
